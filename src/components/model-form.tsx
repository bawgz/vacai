...
export default function ModelForm() {
  ...
  async function handleUpload(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/upload-url", {
      method: "POST",
      body: JSON.stringify({ fileName: `${modelId}.zip` }),
    });
    ...
    const isSuccessfulyCreated = await createModel(
      modelId,
    ...
    <>
      ...
      <form onSubmit={handleUpload}>
        <input
          ...
          required
          className="block w-full p-2 border border-gray-300 rounded-md text-black my-2"
        />
        <select
          name="class"
          id="class"
        ...
        <input
          ...
          multiple
          required
          className="block w-full p-2 border border-gray-300 rounded-md cursor-pointer focus:outline-none my-2 text-black bg-white"
        />
        <button
          type="submit"
          disabled={isLoading}
